...
** we need to check admin attemptin to FIDE
** we need to move admin email, currently "wan_ahmad_ardie@yahoo", to env variable
* Some limitations:
...
* Cpanel:
** cPanel is just a GUI tools for non-technical person to manage the server. They don't know command line. So, what you could do from terminal, you could do it in cPanel. Google has some tutorial for how to deploy the Flask in cPanel, or you can ask the owner's hosting support team to guide you that's the resources you can utilise.
** You can publish your python app with cpanel,just need to make sure the hosting provider is using cloudlinux on their hosting, typically the "84x faster WordPress Hosting" will have it. How do I set up a Python web application? – cPanel https://share.google/GodRzV3LNS1fgoUOh
